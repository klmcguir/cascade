...
import pandas as pd
import os
from . import lookups, load


def pick_comp_order_plus_cuelevel_bhv_mod(cell_cats,
                                 cell_sorter,
                                 mod_vec=None,
                                 bhv_type='speed',
                                 bhv_baseline_or_stim='baseline',
                                 fix_onset_bhv=True):
    """Move cells (sorted by componenets) into a user defined order.
    Sort within components GROUPS by speed mod.
    ...
    # Load running corr and get mean correlation driven stagedays
    if mod_vec is None:
        if len(cell_cats) == 1799:
            # corr_df_2s = pd.read_pickle(
            #     os.path.join(lookups.coreroot, f'onsets_{bhv_baseline_or_stim}_on{bhv_type}_corr_2s_df.pkl')
            #     )
            corr_df_2s = pd.read_pickle(os.path.join(lookups.coreroot, 'onsets_baseline_bhvon_speed_index_2s_df.pkl'))
            corr_vec = corr_df_2s[f'mean_bhvon_speed_index_2s_11stage'].values
            # corr_vec = corr_df_2s[f'mean_on{bhv_type}_corr_2s_stageday'].values
        elif len(cell_cats) == 445:
            fix_tag = 'on' if fix_onset_bhv else 'off'
            corr_df_2s = pd.read_pickle(
                os.path.join(lookups.coreroot, f'offsets_{bhv_baseline_or_stim}_{fix_tag}{bhv_type}_corr_2s_df.pkl')
                )
            corr_vec = corr_df_2s[f'mean_{fix_tag}{bhv_type}_corr_2s_stageday'].values
        else:
    ...
    # pick the correct cue groups based on vector length
    if len(cell_cats) == 1799:
        cat_groups = lookups.tuning_groups['rank9_onset']
    elif len(cell_cats) == 445:
        cat_groups = lookups.tuning_groups['rank8_offset']
    else:
        raise NotImplementedError

    # sort correlations by component first
    corr_sorted = corr_vec[new_sort_order]

    # reorder your sort inds by running corr value within a comp category
    sorted_chunks = []
    for cat_gr in cat_groups.values():
        cat_boo = np.isin(cats_sorted, cat_gr)
...
def pick_comp_order_plus_bhv_mod(cell_cats,
                                 cell_sorter,
                                 mod_vec=None,
                                 bhv_type='speed',
                                 bhv_baseline_or_stim='baseline',
