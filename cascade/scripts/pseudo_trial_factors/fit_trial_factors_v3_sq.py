    ...
    for mouse in np.unique(mouse_vec):
        ...
        for c, di in tqdm(enumerate(days), desc=f'{mouse}: fitting day #', total=len(days)):
            ...
            for c2, di2 in enumerate(runs):
                ...
                # use the shape to get a vector of zeros and set all values after the first second to 1
                new_temporal_factors = np.zeros(new_temporal_factors.shape)
                new_temporal_factors[16:, :] = 1
