        ...
        for c, di in tqdm(enumerate(days), desc=f'{mouse}: fitting day #', total=len(days)):
            ...
            for c2, di2 in enumerate(runs):
                ...
                new_temporal_factors = np.nanmean(stage_scale_traces[:, :, :],
                                                  axis=2).T  # take the mean shape then normalize to max
                ...
                # scale cell factors by the max per stage
                # weight the cell factor by what that comp was doing on that stage
                scale_level = np.nanmax(stage_scale_traces[:, :, stage_ind], axis=1)
                assert len(scale_level) == new_cell_factor.shape[1]
                assert np.max(scale_level) <= 1
                new_cell_factor = new_cell_factor / scale_level
                new_cell_factor[~np.isfinite(new_cell_factor)] = 0  #fix divide by zero

                # dividing by the max per stage accounts for setting the temporal factor to 1. i.e. 
                # if a temporal factor had a low weight 0.2 we have to divide by 0.2 to get to 1. 
