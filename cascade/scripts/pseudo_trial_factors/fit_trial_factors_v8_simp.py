"""Script to fit trial factors using factors from unwrapped models."""
import numpy as np
    ...
    for mouse in np.unique(mouse_vec):
        ...
        for c, di in tqdm(enumerate(days), desc=f'{mouse}: fitting day #', total=len(days)):
            ...
            for c2, di2 in enumerate(runs):
                ...
                new_temporal_factors = stage_scale_traces[:, :, stage_ind].T
                new_temporal_factors_normed = new_temporal_factors ## NOT NORMING ANYMORE
