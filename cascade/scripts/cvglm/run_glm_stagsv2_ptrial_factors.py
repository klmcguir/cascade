...
def main(version='_v2_avg', loss_type='poisson', activation='exp', force=False, verbose=False, test=False):
    ...
    for meta in meta_list:
        ...
        X_temp = pd.concat([design_df, new_meta_df], axis=1)
        X_temp.drop(columns=interaction_cols, inplace=True)
        assert not X_temp.isna().any().any()

        # stage interactions
        interaction_cols = cas.lookups.staging['parsed_11stage']
        interaction_names = [s.replace(' ', '_') for s in cas.lookups.staging['parsed_11stage_T']]
        base_cols = X_temp.columns
        for rep in base_cols:
            for inter, intern in zip(interaction_cols, interaction_names):
                meta_vec = np.zeros(len(meta))
                meta_vec[meta.parsed_11stage.isin([inter])] = 1
                new_meta[f'{rep}_x_{intern}'] = X_temp[rep].values * meta_vec
        X = pd.DataFrame(data=new_meta, index=design_df.index)
    ...
    for mod, rr, y_mat_list in zip(models, ranks, y_mat_mod_list):
        ...
        df_list_ddev = []
        total_model_devex = []
        save_folder = cas.paths.analysis_dir(f'tca_dfs/TCA_factor_fitting{version}/{mod}/cvglm{glm_tag}')
        ...
        for Xdf, Ydf in tqdm(zip(design_mat_list, y_mat_list), desc=f'{mod} GLM', total=len(y_mat_list)):
            ...
            plt.title(f'{mouse}')
            plt.savefig(os.path.join(save_folder, f'{mod}_rank{rr}_{mouse}_cv_model_selection.png'),
                        bbox_inches='tight')
            plt.close('all')
            ...
            this_ecdf = ecdf(x)
            logger.info(f'{mouse}: Test set: mean deviance explained = {np.mean(selected_fit_qual_test)}')
            [
                logger.info(f'{mouse}: Test set: component {compi + 1}: deviance explained = {s}')
            ...
            plt.savefig(os.path.join(save_folder, f'{mod}_rank{rr}_{mouse}_deviance_explained_best_cv_model.png'),
                        bbox_inches='tight')
            plt.close('all')
            ...
            for n_factor in range(Y_test.shape[1]):
                ...
                if test:
                    ...
                    mu_full = cas.cvglm.make_prediction(X_test, w, w0)
                    ax[n_factor].set_title(f'Full model deviance explained: {cas.cvglm.deviance(mu_full, y)[0]}', size=16)
                    plt.suptitle(f'{mouse} {mod} rank {rr} {version} {loss_type} {activation}', size=20, position=(0.5, 0.9))
                    plt.savefig(os.path.join(save_folder,
                                             f'{mouse}_{mod}_rank{rr}{version}_{loss_type}_{activation}_traces.png'),
                                bbox_inches='tight')
            ...
            index = pd.MultiIndex.from_arrays(
                [[mouse] * len(full_w), np.arange(1,
                                                  len(full_w) + 1)], names=['mouse', 'component'])
