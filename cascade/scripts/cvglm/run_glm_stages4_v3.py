...
def main(version='_v3_avg_noT0', loss_type='poisson', activation='exp', force=False, verbose=False, test=False):
    ...
    y_mat_mod_list = []
    error_list = []
    for mc, mod in enumerate(model_list):  # for your 4 models
        y_mat_list = []
        for c, (meta, mouse_mod) in enumerate(zip(meta_list, mod['pseudo_trialfactors'])):  # for your 7 mice
            new_y = {}
            ...
            if y_df.isna().all().any():
                bad_comps = y_df.isna().all().keys().values[y_df.isna().all().values]
                error_list.append(f'{cas.utils.meta_mouse(meta)}: Missing full {bad_comps}. Set to zero.')
                y_df.loc[:, y_df.isna().all()] = 0
            drop_bool = y_df.isna().any(axis=1)
            if drop_bool.sum() > 0:
                bad_comps = y_df.isna().any(axis=0).keys().values[y_df.isna().any(axis=0).values]
                error_list.append(f'{cas.utils.meta_mouse(meta)}: {models[mc]} {bad_comps} filled {drop_bool.sum()} NaN trials.')
                # design_mat_list[c] = design_mat_list[c].loc[~drop_bool]
                # meta_list[c] = meta.loc[~drop_bool]
                # y_df = y_df.loc[~drop_bool]
                y_df = y_df.fillna(0)
    ...
    for mod, rr, y_mat_list in zip(models, ranks, y_mat_mod_list):
        ...
        if len(error_list) > 0:
            logger.info(f'Preprocessing updates start:')
            [logger.error(s) for s in error_list]
            logger.info(f'Preprocessing updates end.\n')
        ...
        for Xdf, Ydf, meta in tqdm(zip(design_mat_list, y_mat_list, meta_list), desc=f'{mod} GLM', total=len(y_mat_list)):
            ...
            for stagi in ['early_learning', 'late_learning', 'early_reversal', 'late_reversal']:
                ...
                if test:
                    # try to get a sense for where your dprime modulation effect is coming from
                    plt.figure(figsize=(15,4))
                    # plt_cols = [s for s in kept_cols if 'same' in s or 'diff' in s]
                    # for col in plt_cols:
                    #      plt.plot(Xdf_sub[col].values[fitInd], label=col, alpha=0.2, marker='o')
                    plt.plot(Xdf_sub.dprime_run.values[fitInd], label='dprime_run train', alpha=0.7, marker='o')
                    plt.plot(Xdf_sub.dprime_run.values[testInd], label='dprime_run test', alpha=0.7, marker='o')
                    plt.ylim([-0.1, 1.1])
                    plt.xlabel('trial number (train set)')
                ...
                plt.title(f'{mouse}')
                plt.savefig(os.path.join(save_folder_ms, f'{mod}_rank{rr}_{mouse}_{stagi}_cv_model_selection.png'),
                            bbox_inches='tight')
                plt.close('all')
                ...
                for n_factor in range(Y_test.shape[1]):
                    ...
                    # do groups of filter together, i.e., all licking, all_running
                    lick_cols = np.where(['lick' in s for s in kept_cols])[0]
                    speed_cols = np.where(['speed' in s for s in kept_cols])[0]
                    extra_devex_cols = ['lick_agg', 'speed_agg']
                    grouped_cols = [lick_cols, speed_cols]
                    for group_ablation in grouped_cols:
                        X_ablated = deepcopy(X_test)
                        X_ablated[:, group_ablation] = 0
                        mu_full = cas.cvglm.make_prediction(X_test, w, w0)
                        mu = cas.cvglm.make_prediction(X_ablated, w, w0)  # this should be done on test data
                        ddev_per_y_w.append(cas.cvglm.deviance(mu_full, y)[0] - cas.cvglm.deviance(mu, y)[0])
                        fdev_per_y_w.append(1 - cas.cvglm.deviance(mu, y)[0] / cas.cvglm.deviance(mu_full, y)[0])
                ...
                data = {k: fac_dev_breakdown[c, :] for c, k in enumerate(kept_cols.to_list() + extra_devex_cols)}
        ...
        beta_df = pd.DataFrame(data=np.stack(all_mdev_df['beta_w'].values, axis=0),
                            columns=all_mfdev_df.columns[:-2], index=added_col_df.index)
