...
def run_unwrapped_tca(thresh=4, force=False, verbose=False):
    ...
    if os.path.isfile(cas.paths.analysis_file(f'tca_ensemble_v{thresh}_nneg_nT0_20210209.npy', 'tca_dfs')) and not force:
        return
    ...
    # offset
    best_cue_resp = np.nanmax(data_dict[f'v{thresh}_norm_off_nneg_nT0'], axis=2)
    nneg = np.sum(best_cue_resp >= 0, axis=1)
    neg = np.sum(best_cue_resp < 0, axis=1)
    ...
    data_dict_path = cas.paths.analysis_file(f'input_data_v{thresh}_nneg_nT0_20210209.npy', 'tca_dfs')
    ...
    np.save(cas.paths.analysis_file(f'tca_ensemble_v{thresh}_nneg_nT0_20210209.npy', 'tca_dfs'), ensemble, allow_pickle=True)
    ...
    for k, v in ensemble.items():
        ...
        plt.savefig(cas.paths.analysis_file(f'{k}_nneg_nT0.png', 'tca_dfs/TCA_qc'), bbox_inches='tight') 
    ...
    for k, v in ensemble.items():
        for rr in range(5, 20):
            ...
            plt.savefig(cas.paths.analysis_file(f'{k}_rank{rr}_facs_nneg_nT0.png', f'tca_dfs/TCA_factors/{k}_nneg_nT0'), bbox_inches='tight')
    ...
    for mod, mmod in zip([f'v{thresh}_norm_on', f'v{thresh}_norm_off'], [f'v{thresh}_on_mouse', f'v{thresh}_off_mouse']):
        ...
        for i in range(1,4):
            ...
            plt.savefig(
                cas.paths.analysis_file(f'{mod}_rank{heatmap_rank}_heatmap_nneg_nT0.png', f'tca_dfs/TCA_heatmaps/v{thresh}_nneg_nT0'),
                bbox_inches='tight')
