...
for date in [20210307, 20210215]:
    ...
    for mod in ['v4i10_norm_on_noT0', 'v4i10_norm_off_noT0', 'v4i10_scale_on_noT0', 'v4i10_scale_off_noT0']:
        # mod = 'v4i10_norm_on_noT0'

        for bot, top in zip([6, 9, 9, 6, 8, 7], [12, 15, 9, 20, 8, 7]):
            # bot = 9
            # top = 9
            add_tuning = True
            iter_to_plot = 10
            ...
            plt.suptitle(f'{mod}, rank {bot}-{top}, temporal factors\nhierarchical clustering',
                         size=20,
                         position=(0.5, 0.95))
            plt.savefig(cas.paths.analysis_file(f'{mod}_rank{bot}-{top}_hierclusTemp_resc{suffix}', f'tca_dfs/TCA_results/{mod}/{date}/clustering_factors/'),
                        bbox_inches='tight')
            ...
            for i in range(iter_to_plot):
                ...
                try:
                    ...
                    plt.suptitle(f'{mod}, rank {bot}-{top}, iteration {i+1}\ntemporal factors, hierarchical clustering',
                                 size=20,
                                 position=(0.5, 0.95))
                    plt.savefig(cas.paths.analysis_file(f'{mod}_rank{bot}-{top}_iter{i+1}_hierclusTemp_resc{suffix}',
            ...
            if bot <= 6 and top >= 12:
                for i in range(6, 13):
                    rank_boo = np.isin(rank_num, i)[sorter2]

                    iter_hues = []
                    for h in sort_hues:
                        iter_hues.append(np.array(h)[rank_boo])

                    # try:
                    sns.clustermap(test[:, sorter2][:, rank_boo].T,
                                    col_cluster=False,
                                    row_cluster=False,
                                    row_colors=iter_hues,
                                    method='ward')

                    plt.suptitle(f'{mod}, rank {i}\ntemporal factors, hierarchical clustering',
                                    size=20,
                                    position=(0.5, 0.95))
                    plt.savefig(cas.paths.analysis_file(f'{mod}_rank{bot}-{top}_rank{i}_hierclusTemp_resc{suffix}',
                                                        f'tca_dfs/TCA_results/{mod}/{date}/clustering_factors_byrank/'),
                                bbox_inches='tight')
                # except:
                #     print(f'skipped iteration {i}')
        plt.close('all')
        ...
        for k, v in ensemble.items():
            ...
            fig, ax = plt.subplots(2,2, figsize=(7,7), sharex='row', sharey='col')
            ax = ax.reshape([2,2])
            ...
            plt.savefig(cas.paths.analysis_file(f'{k}_obj_sim2{suffix}', f'tca_dfs/TCA_results/{mod}/{date}/TCA_qc'), bbox_inches='tight')
        plt.close('all')
        ...
        for k, v in sort_ensembles.items():
            ...
            for rr in range(5, 20):
                # manually sort component order for visualization
                # if '_on' in mod and rr == 9:
                #     plot_facs = v.results[rr][0].factors.rebalance()
                #     plot_facs[0][:, cas.lookups.fixed_component_sort['rank9_onsets']]
                ...
                plt.savefig(cas.paths.analysis_file(f'{k}_rank{rr}_facs{suffix}', f'tca_dfs/TCA_results/{mod}/{date}/TCA_factors/'), bbox_inches='tight')
            plt.close('all')
        ...
        #     plt.savefig(
        #         cas.paths.analysis_file(f'{mod}_rank{heatmap_rank}_heatmap{suffix}', f'tca_dfs/TCA_results/{mod}/{date}/TCA_heatmaps/'),
        #         bbox_inches='tight')
        #     plt.close('all')