...
def run_unwrapped_tca(thresh=4, force=False, verbose=False):
    ...
    if os.path.isfile(cas.paths.analysis_file(f'tca_ensemble_v{thresh}_resc_20210209.npy', 'tca_dfs')) and not force:
        return
    ...
    # --------------------------------------------------------------------------------------------------
    data_dict = {
        f'v{thresh}_on_resc': on_mega_tensor_flat,
        f'v{thresh}_off_resc': off_mega_tensor_flat,
        f'v{thresh}_norm_on_resc': on_mega_tensor_flat_norm,
        f'v{thresh}_norm_off_resc': off_mega_tensor_flat_norm,
        f'v{thresh}_speed_on_resc': on_mega_tensor_flat_run,
        f'v{thresh}_speed_off_resc': off_mega_tensor_flat_run,
        f'v{thresh}_speed_norm_on_resc': on_mega_tensor_flat_run_norm,
        f'v{thresh}_speed_norm_off_resc': off_mega_tensor_flat_run_norm
    }
    # add mouse and cell data to dict
    data_dict[f'v{thresh}_off_mouse_resc'] = off_mega_mouse_flat
    data_dict[f'v{thresh}_on_mouse_resc'] = on_mega_mouse_flat
    data_dict[f'v{thresh}_off_cell_resc'] = off_mega_cell_flat
    data_dict[f'v{thresh}_on_cell_resc'] = on_mega_cell_flat
    ...
    # onset
    best_cue_resp = np.nanmax(data_dict[f'v{thresh}_norm_on_resc'], axis=2)
    ...
    # offset
    best_cue_resp = np.nanmax(data_dict[f'v{thresh}_norm_off_resc'], axis=2)
    nneg = np.sum(best_cue_resp >= 0, axis=1)
    neg = np.sum(best_cue_resp < 0, axis=1)
    ...
    mdict_set = [data_dict[f'v{thresh}_off_mouse_resc'], data_dict[f'v{thresh}_on_mouse_resc']]
    ddict_set = [data_dict[f'v{thresh}_off_resc'], data_dict[f'v{thresh}_on_resc']]
    new_scaled_keys = [f'v{thresh}_scale_off_resc', f'v{thresh}_scale_on_resc']
    ...
    data_dict_path = cas.paths.analysis_file(f'input_data_v{thresh}_resc_20210209.npy', 'tca_dfs')
    ...
    np.save(cas.paths.analysis_file(f'tca_ensemble_v{thresh}_resc_20210209.npy', 'tca_dfs'), ensemble, allow_pickle=True)
    ...
    # plot heatmap
    for mod, mmod in zip([f'v{thresh}_norm_on_resc', f'v{thresh}_norm_off_resc'], [f'v{thresh}_on_mouse_resc', f'v{thresh}_off_mouse_resc']):
        ...
        for i in range(1,4):
            ...
            plt.savefig(
                cas.paths.analysis_file(f'{mod}_rank{heatmap_rank}_heatmap.png', f'tca_dfs/TCA_heatmaps/v{thresh}_resc'),
                bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    run_unwrapped_tca(thresh=int(args.thresh), force=args.force, verbose=args.verbose)
