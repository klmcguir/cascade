...
words = words = ['carter' if m == 'OA27' else 'desperate' for m in mice]
trace_type = 'zscore_day'
folder_name = 'cvGLM pillow interactions'
group_by = 'all3'
nan_thresh = 0.95
score_threshold = 0.8
...
for rank_num in [10, 15]:
    ...
    for m, wi, in zip(mice, words):
        ...
        # make sure your trial factors are positive
        fac1_df = fac1_df.abs()
        ...
        #TODO go through and rename psy1 to reflect the rewarded cues pre and post rather than the ori

        # add a binary column for possible category/semantic representations
        new_meta_df1, p_cols, i_cols, cs_cols = cas.glm.learning_columns_df_dec2020(m, meta1_df)
        new_meta_df1 = cas.glm._add_hmm_to_design_mat(new_meta_df1, meta1_df)  # add engagement from HMM

        # start design matrix
        design_X = pd.concat([new_meta_df1.loc[:, p_cols],
                              new_meta_df1.loc[:, cs_cols],
                              new_meta_df1.loc[:, ['choice', 'reward', 'prev_reward', 'prev_punish']],
                              new_meta_df1.loc[:, ['hmm_engaged']],
                              # new_meta_df1.loc[:, pillow_cols],
                              new_meta_df1.loc[:, i_cols]
                              ], axis=1)
        design_X = design_X.drop(columns=['dp_plus', 'dp_minus', 'dp_neutral'])
        ...
        design_X['pre_licks'] = meta1_df['pre_licks']
        design_X['pre_speed'] = meta1_df['pre_speed']
        design_X['post_licks'] = meta1_df['post_licks']
        design_X['post_speed'] = meta1_df['post_speed']

        # add pillow in and rename ac
        for s in ['plus', 'minus', 'neutral']:
            design_X[f'{cas.lookups.lookup_mm[m][s]}_pillow'] = \
                psy1_df[f'ori_{cas.lookups.lookup[m][s]}_pillow']
        design_X['prev_reward_pillow'] = psy1_df['prev_reward_pillow']
        ...
        new_meta = {}
        # TODO add in pillow factors of interest
        base_cols = ['hmm_engaged', 'anticipatory_licks', 'pre_licks', 'post_licks',
                     'speed', 'pre_speed', 'post_speed', 'prev_reward', 'prev_punish',
                     'becomes_unrewarded_pillow', 'becomes_rewarded_pillow', 'prev_reward_pillow'
                     ]
        interaction_cols = ['initial_plus', 'initial_minus', 'initial_neutral',
    ...
    # save
    all_mddev_df.to_pickle(os.path.join(save_folder, 'cvGLM_delta_deviance_df.pkl'))
    all_mfdev_df.to_pickle(os.path.join(save_folder, 'cvGLM_fractional_deviance_df.pkl'))
    all_mdev_df.to_pickle(os.path.join(save_folder, 'cvGLM_model_performance_df.pkl'))
