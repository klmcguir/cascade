...
for mi, wi in zip(mice, words):
    ...
    bias_df = cas.bias.get_bias_from_model_simple(meta, tensor, model, model_rank=rank_num,
    ...
    plt.savefig(os.path.join(save_folder, f'{mi}_rank_{rank_num}_TCAfull_mean_cue_response.pdf'), bbox_inches='tight')
    ...
    plt.legend(bbox_to_anchor=(1.05, 1))
    plt.savefig(os.path.join(save_folder, f'{mi}_rank_{rank_num}_TCAfull_mean_cue_response_raw.pdf'), bbox_inches='tight')
    ...
    plt.legend(bbox_to_anchor=(1.05, 1))
    plt.savefig(os.path.join(save_folder, f'{mi}_rank_{rank_num}_TCAfull_FC_bias.pdf'), bbox_inches='tight')
