...
def weighted_avg_first100(
    ...
    for aci in adapting_comps:
        ...
        ax1.set_title(
            '{}: Component {}: {}: Ensemble average sustained responses (first 100 trials per day)'.format(mouse, aci,
                                                                                                           stim_or_noise),
            size=16)
...
def tca_first100(
    ...
    # get useful variables
    pupil = meta_stim['pupil'].values[firstboo]
    if 'dprime_run' not in meta_stim.columns:
        meta_stim = utils.add_dprime_run_to_meta(meta_stim)
    dp100 = meta_stim['dprime_run'].values[firstboo]
    ...
    for aci in adapting_comps:
        ...
        ax1.plot([rev_ind, rev_ind], [y_min, y_max], '--k')
        ax1.set_title(
            '{}: Component {}: {}: TCA model responses (first 100 trials per day)'.format(mouse, aci, stim_or_noise),
            size=16)
        ax1.set_xlabel('trial number', size=14)
        ax1.set_ylabel('response amplitude (weighted z-score)', size=14)
...
def sustainedness_daily_mean_comp_plots(
    ...
    for ori in ['plus', 'minus', 'neutral']:
        ...
        for mi, meti, V in zip(mice, meta_list, model_list):
            ...
            plt.savefig(
                './/{}//Sustainedness plots//{}_{}_{}_{}_sus_avg_stages.pdf'.format(folder, t_or_wa, mi, ori, words[0]),
                bbox_inches='tight')
        #         plt.close('all')
        ...
        plt.title(
            '{}: '.format(ori) + 'Sustainedness vs $\Delta$$SI_{lrn-lrn}$' + ': R={}, {}={}'.format(round(r[0], 2),
                                                                                                    '$-1*log_{10}(p)$',
                                                                                                    round(-1 * np.log10(
                                                                                                        r[1]), 3)))
        plt.savefig('.//{}//{}_{}_{}_SI_learnhdp_v_learnldp.pdf'.format(folder, t_or_wa, ori, words[0]),
                    bbox_inches='tight')
        ...
        plt.title(
            '{}: '.format(ori) + 'Sustainedness vs $\Delta$$SI_{lrn-naive}$' + ': R={}, {}={}'.format(round(r[0], 2),
                                                                                                      '$-1*log_{10}(p)$',
                                                                                                      round(
                                                                                                          -1 * np.log10(
                                                                                                              r[1]),
                                                                                                          3)))
        plt.savefig('.//{}//{}_{}_{}_SI_learnhdp_v_naive.pdf'.format(folder, t_or_wa, ori, words[0]),
                    bbox_inches='tight')
        ...
        plt.xlabel('sustainedness\n high dprime learning')
        plt.title(
            '{}: '.format(ori) + 'Sustainedness vs $\Delta$$SI_{lrn-rev}$' + ': R={}, {}={}'.format(round(r[0], 2),
                                                                                                    '$-1*log_{10}(p)$',
                                                                                                    round(-1 * np.log10(
                                                                                                        r[1]), 3)))
        plt.savefig('.//{}//{}_{}_{}_SI_learnhdp_v_revldp.pdf'.format(folder, t_or_wa, ori, words[0]),
                    bbox_inches='tight')
        ...
        plt.title(
            '{}: '.format(ori) + 'Sustainedness vs $\Delta$$SI_{rev-rev}$' + ': R={}, {}={}'.format(round(r[0], 2),
                                                                                                    '$-1*log_{10}(p)$',
                                                                                                    round(-1 * np.log10(
                                                                                                        r[1]), 3)))
        plt.savefig('.//{}//{}_{}_{}_SI_revhdp_v_revldp.pdf'.format(folder, t_or_wa, ori, words[0]),
                    bbox_inches='tight')
    ...
    for stages_to_comp in secondary_stages:
        ...
        plt.savefig(
            './/{}//{}_{}_{}_sus_{}_v_{}.pdf'.format(save_dir, t_or_wa, tag, words[0], primary_stage, stages_to_comp),
            bbox_inches='tight')
        ...
        for ori in ['plus', 'minus', 'neutral']:
            ...
            plt.ylabel('sustainedness coeff {}'.format(stages_to_comp), size=14)
            plt.xlabel('sustainedness coeff {}'.format(primary_stage), size=14)
            plt.title('Sustainedness coeff {} vs Sustainedness coeff {}\n\n'.format(primary_stage, stages_to_comp),
                      size=16)
            plt.savefig('.//{}//{}_{}_{}_sus_initial_{}_{}_v_{}.pdf'.format(save_dir, t_or_wa, tag, words[0], ori,
                                                                            primary_stage, stages_to_comp),
                        bbox_inches='tight')
    ...
    for stages_to_comp in secondary_stages:
        ...
        plt.savefig(
            './/{}//{}_{}_{}_trans_{}_v_{}.pdf'.format(save_dir, t_or_wa, tag, words[0], primary_stage, stages_to_comp),
            bbox_inches='tight')
        ...
        for ori in ['plus', 'minus', 'neutral']:
            ...
            plt.title('Transientness coeff {} vs Transientness coeff {}\n\n'.format(primary_stage, stages_to_comp),
                      size=16)
            plt.savefig('.//{}//{}_{}_{}_trans_initial_{}_{}_v_{}.pdf'.format(save_dir, t_or_wa, tag, words[0], ori,
                                                                              primary_stage, stages_to_comp),
                        bbox_inches='tight')
    ...
    for ori in ['plus', 'minus', 'neutral']:
        ...
        sns.regplot(x=total_sus['high_dp learning'].iloc[oriboo], y=regtester['delta_sus'], marker='.',
                    color=color_dict[ori], dropna=True)
    ...
    for ori in ['plus', 'minus', 'neutral']:
        oriboo = ori_vec == 'initial {}'.format(ori)
        r = sp.stats.pearsonr(total_sus['high_dp learning'].loc[keep_bool & oriboo],
                              tester['delta_sus'].loc[keep_bool & oriboo])
        ps.append(r[1])
        rs.append(r[0])
    ...
    plt.savefig('.//{}//{}_{}_{}_SI_learnhdp_v_learnldp.pdf'.format(folder, t_or_wa, tag, words[0]),
                bbox_inches='tight')
    ...
    for ori in ['plus', 'minus', 'neutral']:
        ...
        sns.regplot(x=total_sus['high_dp reversal1'].iloc[oriboo], y=regtester['delta_sus'], marker='.',
                    color=color_dict[ori], dropna=True)
    ...
    for ori in ['plus', 'minus', 'neutral']:
        oriboo = ori_vec == 'initial {}'.format(ori)
        r = sp.stats.pearsonr(total_sus['high_dp reversal1'].loc[keep_bool & oriboo],
                              tester['delta_sus'].loc[keep_bool & oriboo])
        ps.append(r[1])
        rs.append(r[0])
